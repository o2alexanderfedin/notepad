...
// Import the browser file system handler (auto-registers with the adapter registry)
import './file-system-handler.js';

// Import the file system factory and utilities
import {
  createFileSystem,
...
async function handleOpenFile() {
  try {
    ...
    if (result) {
      ...
      appState.currentHandle = result.handle;
...
async function handleSave() {
  try {
    ...
    if (result) {
      ...
      // Update file name if this was a new file
      if (result.name) {
        appState.currentFileName = result.name;
      }
      setModified(false);
...
/**
 * Shows the unsupported browser warning and disables file operations.
 */
function showBrowserWarning() {
  ...
  setStatus('Browser not supported - file operations disabled');
}
